...
export class TrackmaniaClient {
  ...
  async clubRoom(offset = 0, length = 100) {
    return await this.get<ClubRoomResponse>(`/club/room?offset=${offset}&length=${length}`, true);
  }
  async servers(serverAccountId: string) {
    return await this.get<Server>(`/servers/${serverAccountId}`);
  }
}
...
export interface ClubRoomResponse {
